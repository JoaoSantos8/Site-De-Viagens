...
$escolhaAnteriorOrigem = isset($_POST['origem']) ? mysqli_real_escape_string($con, $_POST['origem']) : '';
...
<div class="w-75 p-5 m-5">
    <h1>Edita Viagem</h1>
    <form class="p-5 mt-5" action="confirmarEditarViagem.php" method="post" enctype="multipart/form-data" id="formulario">

        <div class="mb-3 p-3">
            <label for="nomeHotel" class="form-label labelNome">TELEFONE PARA SUPORTE</label>
            <input type="number" class="form-control" id="inputTel" name="telefone" value="<?php  echo $dados['viagemTelefone'] ?>">
        </div>

        <div class="mb-3 w-50 p-3  float-left">
            <label for="data" class="form-label">DATA</label>
            <input type="date" class="form-control" id="inputDataC" name="dataC" value="<?php  echo $dados['viagemData'] ?>">
        </div>

        <div class="mb-3 w-50 p-3 float-right">
            <label for="data" class="form-label">LUGARES NO AVIÃO</label>
            <input type="number" class="form-control" id="inputLugares" name="lugares" value="<?php  echo $dados['viagemLugares'] ?>">
        </div>
        <br>

        <div class="mb-3 w-50 p-3 float-left">
            <label for="data" class="form-label">HORA QUE PARTE</label>
            <input type="time" class="form-control" id="inputHoraC" name="horaC" value="<?php  echo $dados['viagemIni'] ?>">
        </div>

        <div class="mb-3 w-50 p-3 float-right">
            <label for="data" class="form-label">HORA QUE ATERRA</label>
            <input type="time" class="form-control" id="inputHoraA" name="horaA" value="<?php  echo $dados['viagemChegada'] ?>">
        </div>
        ...
        <select name="origem" id="slect" class="p-3 mt-1 w-25 text-center">
            ...
            while ($dadosO = mysqli_fetch_array($resultd)) { 
                $selected = ($dadosO['destinoNome'] == $escolhaAnteriorOrigem) ? 'selected' : '';
                echo '<option ' . $selected . '>' . $dadosO['destinoNome'] . '</option>';
            }
            ?>
        </select>
        <br>
