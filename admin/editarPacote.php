...
<div class="w-75 p-5 m-5">
    <h1>Edita Pacote</h1>
    <form class="p-5 mt-5" action="confirmarEditarPacote.php" method="post" enctype="multipart/form-data" id="formulario">

        <div class="mb-3 p-3">
            <label for="nomePacote" class="form-label labelNome">NOME - PACOTE</label>
...
?>
        ...
        <select name="viagem" id="slect" class="p-3 mt-1 w-50 text-center">
            <?php
            while ($dadosD = mysqli_fetch_array($resulta)) { 
                $selected = ($dadosD['destinoNome'] == $viagem) ? 'selected' : '';
                echo '<option value="' . $dadosD['viagemPreco'] . '" ' . $selected . '>|   Destino ' . $dadosD['destinoNome'] . '   |   Preço ' . $dadosD['viagemPreco'] . '€   |   Data ' . date('d/m/Y', strtotime($dados['viagemData'])) . "   |" . '</option>';
}
        ...
        <select name="hotel" id="slecte" class="p-3 mt-1 w-50 text-center">
            <?php
